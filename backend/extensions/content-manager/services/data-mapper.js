...
const HIDDEN_CONTENT_TYPES = [
  ...
  'plugins::checkout.calorie',
  'plugins::checkout.cart',
  'plugins::checkout.cart-items',
];

const isHidden = (contentType) => {

  if (contentType.isDisplayed !== undefined) {
     return !contentType.isDisplayed;
  }

  return startsWith('strapi::', contentType.uid) || HIDDEN_CONTENT_TYPES.includes(contentType.uid);
};
